...
def init_four_layer_convnet(weight_scale=1e-3, bias_scale=0, input_shape=(3, 32, 32),
                           num_classes=10, num_filters=32, filter_size=5):
...
def convrelu_convrelupool(X, model, y=None, reg=0.0):
...
def init_convrelu_convrelupool(weight_scale=1e-3, bias_scale=0, input_shape=(3, 32, 32),
                           num_classes=10, num_filters=32, filter_size=5):
  """
  Initialize the weights for a two-layer ConvNet.
...
def BP_two_layer_convnet(X, model, y=None, reg=0.0):
  """
  Compute the loss and gradient for a simple two-layer ConvNet. The architecture
  is conv-relu-pool-affine-softmax, where the conv layer uses stride-1 "same"
  convolutions to preserve the input size; the pool layer uses non-overlapping
  ...
  pool_param = {'pool_height': 1, 'pool_width': 2, 'stride': 2}
  # Compute the forward pass
  a1, cache1 = conv_relu_pool_forward(X, W1, b1, conv_param, pool_param)
  scores, cache2 = affine_forward(a1, W2, b2)
  ...
  # Compute the backward pass
  #data_loss, dscores = softmax_loss(scores, y)
  data_loss = 0.5*(np.sum(np.square(scores-y)))/N
  dscores = (scores-y)/N  # gradient of mse wrt to score
...
def BP_init_two_layer_convnet(weight_scale=1e-3, bias_scale=0, input_shape=(1, 1, 1000),
                           num_classes=2, num_filters=32, filter_size=5):
  """
  Initialize the weights for a two-layer ConvNet.
  ...
  model['b1'] = bias_scale * np.random.randn(num_filters)
  model['W2'] = weight_scale * np.random.randn((num_filters*H*(W-filter_size+1))/2, num_classes)
  model['b2'] = bias_scale * np.random.randn(num_classes)
  return model



def BP_convrelu_convrelupool(X, model, y=None, reg=0.0):
   # Unpack weights
  W1, b1, W2, b2, W3, b3 = model['W1'], model['b1'], model['W2'], model['b2'], model['W3'], model['b3']
  N, C, H, W = X.shape
...
def BP_init_convrelu_convrelupool(weight_scale=1e-3, bias_scale=0, input_shape=(1, 1, 1000),
                           num_classes=2, num_filters=32, filter_size=5):
  """
  Initialize the weights for a two-layer ConvNet.
  ...
  model['W3'] = weight_scale * np.random.randn((num_filters*H*(W-(2*filter_size)+2))/2, num_classes)
  model['b3'] = bias_scale * np.random.randn(num_classes)
  return model
