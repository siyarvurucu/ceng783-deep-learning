    ...
    for it in xrange(num_iters):
      ...
      if first_it or epoch_end or acc_check:
        ...
        if N > 1000:
          ...
          X_train_subset = X
          y_train_subset = y
        prediction_train = loss_function(X_train_subset, model)
        
        train_err = np.sum(np.square(prediction_train - y_train_subset), axis=1).mean()
        train_err_history.append(train_err)
        
        # evaluate val error
        prediction_val = loss_function(X_val, model)
        val_err = np.sum(np.square(prediction_val - y_val), axis=1).mean()
        ...
        if verbose:
          print ('Finished epoch %d / %d: loss %f, train error: %f, val error %f, lr %e'
