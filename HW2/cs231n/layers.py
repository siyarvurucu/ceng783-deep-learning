...
def affine_forward(x, w, b):
  ...
  # shape N x D.
  x_reshaped = x.reshape(x.shape[0],-1)            # reshaping
    ...
    for j in xrange(F): # jth filter
      ...
      for k in xrange(Hp):
        ...
        for l in xrange(Wp):
          ...
          # FILL IN THIS PART
          # 
          out[i,j,k,l] = np.sum(np.multiply(w[j,:,:,:],padded[i,:,hs:(hs+HH),ws:(ws+WW)])) + b[j]
  cache = x, w, b, conv_param
  return out, cache
    ...
    for j in xrange(F): # jth filter
      ...
      for k in xrange(Hp):
        ...
        for l in xrange(Wp):
          ...
          dw[j,:,:,:] += np.multiply(x_pad[i,:,hs:(hs+HH),ws:(ws+WW)],dout[i,j,k,l])
          dx[i,:,k,l] += np.sum(np.sum(np.multiply(dout_pad[i,j,hs:(hs+HH),ws:(ws+WW)],np.flip(np.flip(w[j,:,:,:],1),2)),axis=2),axis=1)
  db = np.sum(dout,axis=0).sum(axis=1).sum(axis=1)
    ...
    for j in xrange(C):
      for k in xrange(Hp):
        ...
        for l in xrange(Wp):
          ...
          # FILL IN THIS PART 
          # 
          #a = x[i,j,hs:(hs+HH),ws:(ws+WW)]
          ind = np.unravel_index(np.argmax(x[i,j,hs:(hs+HH),ws:(ws+WW)], axis=None),(HH,WW))
          dx[i,j,hs+ind[0],ws+ind[1]] = dout[i,j,k,l]
  return dx
