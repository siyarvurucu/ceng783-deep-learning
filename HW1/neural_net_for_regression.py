...
class TwoLayerNet(object):
  ...
  We train the network with a MSE loss function and L2 regularization on the
  weight matrices. The network uses a ReLU nonlinearity after the first fully
  connected layer.

  In other words, the network has the following architecture:

  input - fully connected layer - ReLU - fully connected layer - MSE
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    Hidden_state = X.dot(W1) + b1  # N,H = N,D dot D,H 
    Hidden_state[Hidden_state<= 0] = 0
    scores = Hidden_state.dot(W2) + b2  # N,C = N,H dot H,C
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    loss = 0.5*(np.sum(np.square(scores-y)))/N + 0.5*reg*(np.sum(W1*W1)+np.sum(W2*W2))
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    dW2 = Hidden_state.T.dot(dScores)
    dW2 += reg*W2
    grads['W2'] = dW2   # H,C
    db2  = np.sum(dScores,axis=0)
    grads['b2'] = db2
    
    dHidden = np.dot(dScores, W2.T) # N,C dot C,H = N,H
    dHidden[Hidden_state <= 0] = 0  # backprop through relu
    dW1 = X.T.dot(dHidden) # (D,N) dot (N,H) = D,H shape of W1
    dW1 += reg*W1
  ...
  def predict(self, X):
    ...
    ###########################################################################
    Hidden_state=X.dot(self.params['W1'])+self.params['b1']
    Hidden_state[Hidden_state<= 0] = 0
    y_pred = Hidden_state.dot(self.params['W2']) + self.params['b2']
