  ...
  def sample(self, features, max_length=30):
    ...
    if self.cell_type=='rnn':
        for t in xrange(max_length-1):
            embed , cache_embed= word_embedding_forward(captions[:,t],W_embed)
            h,cache_rnn=rnn_step_forward(embed,h,Wx,Wh,b)
            out,cache = temporal_affine_forward(h[:,np.newaxis,:],W_vocab,b_vocab)
            out = out[:,0,:]
            captions[:,t+1] = np.argmax(out,axis=1)    
    else:
        for t in xrange(max_length-1):
            embed , cache_embed= word_embedding_forward(captions[:,t],W_embed)
            h,c,cache_lstm=lstm_step_forward(embed,h,c,Wx,Wh,b)
            out,cache = temporal_affine_forward(h[:,np.newaxis,:],W_vocab,b_vocab)
            out = out[:,0,:]
            captions[:,t+1] = np.argmax(out,axis=1)    
